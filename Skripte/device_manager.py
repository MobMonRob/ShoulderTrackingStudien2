...
# Modifizierter Code aus dem Intel-Realsense-Multicam Beispiel
...
class DeviceManager:
    ...
    def get_infro_to_color_extrinsics(self, frames):
        """
        Get the extrinsics between the infrared imager and the color imager using its frame delivered by the realsense device
        ...
        for (dev_info, frameset) in frames.items():
            serial = dev_info[0]
            device_extrinsics[serial] = frameset[
                rs.stream.infrared,1].get_profile().as_video_stream_profile().get_extrinsics_to(
                frameset[rs.stream.color].get_profile())
        return device_extrinsics
...
if __name__ == "__main__":
    try:
        ...
        infra_extrinsics = device_manager.get_infro_to_color_extrinsics(frames)
        print(device_extrinsics.values())
        print(infra_extrinsics.values())
    finally:
